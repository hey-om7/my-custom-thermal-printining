...
async def run_ble(data, device: Optional[str]):
    ...
    async with BleakClient(address) as client:
        ...
        # Let the physical hardware finish printing before hanging up the BLE connection
        logger.info("⏳ Data sent! Keeping connection alive for 5 seconds to allow physical printing...")
        await asyncio.sleep(5)

        # THE FIX: Commented out the wait for the printer done event so it stops timing out
        # try:
